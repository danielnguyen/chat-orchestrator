    ...
    return [
        {
            ...
            "content": (
                ...
                "For structured_field_values, each element of structured_data.values is "
                "one source observation or value. Preserve each element boundary during "
                "reasoning; do not split or flatten lexical components or numeric "
                "substrings inside one value into separate observations or aggregate "
                "members. For a mean, each direct operand must represent one complete "
                "semantic quantity at the level being averaged. If one observation needs "
                "a bounded mechanical transformation first, request an intermediate "
                "derivation and let the terminal derivation consume its result. A literal "
    ...
    return [
        {
            ...
            "content": (
                ...
                "or lookup when bounded evidence is sufficient. Use exhaustive_review when "
                "the requested result depends on evaluating the selected source's records "
                "relative to one another or establishing a property of the record set, so "
                "an omitted relevant record could change the result. This includes selecting "
                "an extreme, ranking entries, comparing frequency or significance, choosing "
                "the entry that best satisfies a criterion, or otherwise characterizing the "
                "set. The user does not need to say all, every, complete, or exhaustive for "
                "that coverage to be material. Continue to use lookup when retrieving bounded "
                "matching evidence can establish the answer without showing that every "
                "relevant record was examined; mentioning or returning multiple records alone "
                "does not require exhaustive_review. Use unknown only when the "
