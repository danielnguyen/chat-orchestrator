...
def test_model_interpreted_decimals_execute_against_authorized_provenance():
    records = execute_derivations(
        [
            {
                "derivation_id": "mean-1",
    ...
    assert records[0]["canonical_inputs"] == ["0.25", "0.375", "0.5625", "0.625"]
    assert records[0]["canonical_result"] == "0.453125"
    assert records[0]["supporting_evidence_ref_ids"] == ["fact-1"]
